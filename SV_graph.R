...
rendergraph34 <- function(graph34_Htb,graph34_Phi,graph34,graph34_plot){
  ...
  # Interpolate for Tav
  interp_graph34 <- interp_extra_linear(graph34$x_Htb, graph34$Phi, graph34$y_Tav, graph34_Htb, graph34_Phi)
  
  graph34_plot <- graph34_plot +
    geom_point(aes(x = graph34_Htb, y = interp_graph34), size = 3, color = "red") +
    geom_label(aes(x = graph34_Htb, y = interp_graph34, label = graph34_Phi), vjust = -0.5, fill = "pink") +
    geom_segment(aes(x = graph34_Htb, y = interp_graph34, xend = graph34_Htb, yend = 0), color = "blue", size = 0.8, linetype = "dashed") + # vertical to E
    geom_segment(aes(x = graph34_Htb, y = interp_graph34, xend = 0, yend = interp_graph34), color = "blue", size = 0.8, linetype = "dashed") + # vertical to E
    geom_label(aes(x = graph34_Htb, y = 0, label = graph34_Htb), fill = "green") +
    geom_label(aes(x = 0, y = interp_graph34, label = round(interp_graph34,5)), hjust = 0, fill = "green") +
    theme(axis.text.x = element_text(size = 14),
...
rendergraph36 <- function(graph36_H_D,graph36_E1_E2,graph36_E2_E3,graph36,graph36_plot){
  ...
  graph36_plot <- graph36_plot +
    ...
    geom_label(aes(x = interp_E2E3, y = 0, label = round(interp_E2E3/2, 2)), fill = "green")
  return(graph36_plot)
}